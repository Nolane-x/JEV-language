...
const parseVietnameseFrame = (text: string): ControlledFrame | undefined => {
  ...
  match = viRe(
    String.raw`^Dịch vụ\s+đó\s+xóa\s+đúng\s+(\d+)\s+(?:cái\s+)?(?:tệp(?:\s+tin)?|file)\.?import {
  err,
  StructuredError,
  type Result,
} from "../../core-types/src/index.ts";
...
,
  ...
  match = viRe(
    String.raw`^Dịch vụ\s+có\s+được(?:\s+phép)?\s+xóa\s+đúng\s+(\d+)\s+(?:cái\s+)?(?:tệp(?:\s+tin)?|file)\s+không\?import {
  err,
  StructuredError,
  type Result,
} from "../../core-types/src/index.ts";
